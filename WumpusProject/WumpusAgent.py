...
def getMove(sensor):
    ...
    for p in percepts:
        if p != 'U' and up != True:  # bumpcheck clear and up not true so move south or down
            ...
            return move
    for p in percepts:
        if p == 'G': #if senses glitter, grabs gold and calls escape method to finish game
            gold(p)
...
def gold(sensor):
    ...
    if percepts == 'G':
        if htw.glitterCheck(playx,playy,playl) == True:
            escape()
        elif htw.glitterCheck(playx,playy,playl) == False:
         move = 'W'
        if htw.glitterCheck(playx, playy, playl) == True:
            escape()
        elif htw.glitterCheck(playx, playy, playl) == False:
